...
var monthName = new Array("1월", "2월", "3월", "4월", "5월", "6월", "7월", "8월", "9월",
		"10월", "11월", "12월")
var monthName2 = new Array("1월", "2월", "3월", "4월", "5월", "6월", "7월", "8월",
		"9월", "10월", "11월", "12월")
	...
	if (!ns4) {
		...
		if (showToday == 1) {
			oPopup.document.getElementById("lblToday").innerHTML = ""
					...
					// onFocus='this.blur()'>"+
					" style='" + styleAnchor
					+ "' onclick=parent.totoday() onFocus='this.blur()'>"
					+ "오늘 날짜 :  " + yearNow + "년 " + ""
					+ monthName[monthNow].substring(0, 3) + " " + "" + dateNow
					+ "일 " + // 일
					"</div>";
		}
	}
...
function constructMonth() {
	...
	if (!monthConstructed) { // 월 이전 월 링크
		...
		nStartingMonth = (nSelectedMonth - 3) < 1 ? 1 : nSelectedMonth - 3; // 시작월
																			// - 3
																			// 이
																			// 1보다
																			// 작으면
																			// 1로
																			// 고정
		nStartingMonth = nStartingMonth > 6 ? 6 : nStartingMonth; // 시작월이 6보다
																	// 크면 6로 고정
																	// (6 시작 월 +
																	// 목록 숫자 6 =
																	// 12 종료 월)

		var nEndMonth = (nSelectedMonth + 3) > 12 ? 12 : (nSelectedMonth + 3); // 종료월
																				// + 3이
																				// 12보다
																				// 크면
																				// 12로
		...
		for (i = nStartingMonth; i <= nEndMonth; i++) {
			...
			sHTML += " style='cursor:pointer' onClick='parent.selectMonth(" + j
					+ ");event.cancelBubble=true'> " + sName + "월"
			sHTML += "</td></tr>";
			j++;
		}
	...
	if (!yearConstructed) { // 년도 이전 년도 링크
		...
		for (i = (yearSelected - 3); i <= (yearSelected + 3); i++) {
			...
			sHTML += " style='cursor:pointer' onClick='parent.selectYear(" + j
					+ ");event.cancelBubble=true'> " + sName + "년  "
			sHTML += "</td></tr>";
			j++;
		}
...
function constructCalendar() {
	...
	for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++) {
		...
		// ///////////////////////////////
		dateMessage = "title=' 날짜 선택 : " + yearSelected + "년 "
				+ monthName[monthSelected] + " " + datePointer + "일"
				+ "' onmousemove='window.status=\" 날짜 선택 : " + yearSelected
				+ "년 " + monthName[monthSelected] + " " + datePointer + "일"
				+ "\"' onmouseout='window.status=\"\"' ";

		if ((datePointer == dateNow) && (monthSelected == monthNow)
				&& (yearSelected == yearNow)) { // 현재 오늘 날짜
			sHTML += "<b><div style='" + sStyle
					+ "' onclick='javascript:parent.dateSelected="
					+ datePointer
					+ ";parent.closeCalendar();'><font color=#0000C0> "
					+ datePointer + "</font> </div></b>";
		} else if ((dayPointer % 7 == (startAt * -1) + 1)
				|| (dayPointer % 7 == (startAt * -1))) { // 일요일, 토요일일때
			...
			sHTML += "<div style='" + sStyle
					+ "' onclick='javascript:parent.dateSelected="
					+ datePointer + ";parent.closeCalendar();'>" + datePointer
					+ "</div>";
		}
		sHTML += "";
	...
	oPopup.document.getElementById("spanMonth").innerHTML = " "
			...
			+ " <input type='button'  id='changeMonth'value='▼'  class='cnj_input2' onfocus='this.blur()' onMouseover=\"this.className='cnj_input3'\" onMouseout=\"this.className='cnj_input2'\">"

	// ////// 셀릭트 년도 선택
	oPopup.document.getElementById("spanYear").innerHTML = " "
			+ yearSelected
			+ "년 <input type='button'  id='changeYear'' value='▼'  class='cnj_input2' onfocus='this.blur()' onMouseover=\"this.className='cnj_input3'\" onMouseout=\"this.className='cnj_input2'\">"
...
function popUpCalendar(ctl, ctl2, format) {
	...
	if (bPageLoaded) {
		...
		ctop = fixedY == -1 ? ctl.offsetTop + ctl.offsetHeight + toppos
				: fixedY;
